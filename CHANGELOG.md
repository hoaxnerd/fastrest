...
### Fixed
- Permission composition with nested `&`/`|`/`~` operators — `_AND`, `_OR`, `_NOT` now correctly resolve composed permission objects (e.g. `IsAuthenticated() & (IsOwner() | IsAdminUser())` no longer raises `TypeError`)
